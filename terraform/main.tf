...
resource "aws_api_gateway_rest_api" "example_api" {
  name = "ifndef_gateway"
  description = "Gateway for ifndef website"
}
...
resource "aws_api_gateway_integration" "example_api_method-integration" {
  ...
  type = "AWS_PROXY"
  uri = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${aws_lambda_function.example_test_function.arn}/invocations"
